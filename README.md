...
- Диктовка не самодостаточна и не заработает на чужой машине прямо сейчас.
  `TranscriberBridge` запускает воркер интерпретатором из
  `~/Library/Application Support/WhisperDictation/runtime/.venv/bin/python` —
  окружением, поднятым ещё для старого отдельного приложения WhisperDictation,
  — а `Resources/worker/cyclop_worker.py` импортирует из него пакет
  `whisper_dictation`. Нужен не питон с mlx-whisper вообще, а именно это
  окружение целиком; настройка `dictation.python` в `UserDefaults` меняет
  только путь к бинарнику интерпретатора и отсутствие пакета не лечит. На
  машине, где WhisperDictation никогда не стоял, при первом же удержании
  правого ⌥ вкладка «Диктовка» покажет ошибку «Не найден рантайм
  распознавания» вместо работы. Упаковка собственного рантайма (venv в
  бандле или что придёт на замену mlx-whisper) в этой ветке не сделана —
...
- Диктовка в Cyclop и старое отдельное приложение WhisperDictation слушают один
  ...
  доступ, оба честно сработают на одно нажатие и текст вставится дважды. Это
  ожидаемо и сразу заметно: значит, тестируешь. Пока диктовка в Cyclop не
  проверена вживую на реальном хоткее (сборка, из которой писался этот файл,
  ни разу не получала Универсальный доступ — замеры памяти сделаны прогоном
  воркера напрямую, см. `docs/dictation-memory.md`), WhisperDictation остаётся
  запущенным. В автозапуске его нет — ни в Объектах входа, ни в LaunchAgents,
  сейчас это просто открытое вручную приложение, которое ничего не выгружает
  и держит модель в памяти сутками (тоже см. `docs/dictation-memory.md`).
  Именно поэтому останавливать его раньше времени не стоит: если в переносе
  всплывёт баг, не пойманный прямым прогоном воркера, включить диктовку
  обратно можно будет только открыв WhisperDictation вручную ещё раз — а не
  просто перезагрузкой. Когда диктовка в Cyclop отработает вживую — минимум
  неделю обычного использования, без двойной вставки и без падений воркера, —
  тогда и только тогда: `osascript -e 'tell application "WhisperDictation" to quit'`.
  Сам `.app` не удалять — он остаётся запасным путём, пока новый код не
  доказал, что справляется без него.
