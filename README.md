...
Доступ к Календарю и диктовка — единственные два разрешения, которые Cyclop
вообще запрашивает. Оба нужны только своей вкладке, и в обоих случаях системный
диалог показывается не при запуске и не при открытии вкладки, а по явному
нажатию кнопки на объясняющем экране внутри неё самой. Не пользуешься ни тем,
ни другим — приложение так и остаётся без разрешений.

Диктовке нужны два: микрофон — чтобы слышать, и Универсальный доступ — чтобы
видеть нажатие правого ⌥ где угодно в системе (это глобальный `CGEventTap`,
без Универсального доступа он не получает событий) и вставлять распознанный
текст под курсором в чужом окне. Кнопка «Allow» на экране вкладки запрашивает
оба сразу — `AVCaptureDevice.requestAccess` и `AXIsProcessTrustedWithOptions`.
...
- Диктовка в Cyclop и старое отдельное приложение WhisperDictation слушают один
  ...
  работать и в автозапуске: остановить его раньше значило бы остаться совсем
  без диктовки, если в переносе всплывёт баг, не пойманный прямым прогоном.
  Когда диктовка в Cyclop отработает вживую — минимум неделю обычного
  использования, без двойной вставки и без падений воркера, — тогда и только
  тогда: `osascript -e 'tell application "WhisperDictation" to quit'`, и в
  Системные настройки → Основные → Объекты входа убрать его из автозапуска.
