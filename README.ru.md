...
**[Скачать последнюю версию](https://github.com/Fuck4ik/cyclop/releases/latest)** —
macOS 15 или новее. Разрешения запрашиваются только при использовании нужной
вкладки, [вот как это устроено](#установка).

```
0.0 % CPU в покое  ·  ≈40 МБ + 14 МБ helper  ·  разрешения только по запросу
```
...
| **Диктовка** | Удерживай правый ⌥, говори, отпусти — текст появляется под курсором. Распознавание локальное (Whisper large-v3-turbo через MLX), история расшифровок с поиском и прослушиванием лежит в той же вкладке. Два быстрых нажатия той же клавиши вставляют заранее заданную фразу — ту, которую иначе диктуешь по десять раз на дню |
...
**В Finder — «Копировать полный путь».** Отдельный пункт контекстного меню, не
внутри «Быстрых действий»: правый клик по файлу или папке кладёт в буфер обмена
абсолютный путь без `file://`. Выделено несколько объектов — путей будет
несколько, по одному на строку. Пробелы, кириллица и всё остальное, что бывает
в именах, приезжают как есть. Панель при этом не всплывает и фокус не забирает:
пункт рисует расширение внутри `Cyclop.app`, и работает оно даже когда само
приложение не запущено.

Расширения Finder macOS включает не сама. Один раз: Системные настройки →
Основные → «Объекты входа и расширения» → Расширения Finder → Cyclop. Прямо
туда ведёт строка «Копировать полный путь в Finder» во вкладке настроек — там же
видно, включено оно сейчас или нет.
...
Для диктовки на новом Mac понадобятся Микрофон и Универсальный доступ: первое
нужно, чтобы слышать речь, второе — чтобы видеть правый ⌥ и вставлять текст в
активное приложение. Первая диктовка скачает выбранную модель Whisper; запись
можно начать до окончания загрузки, а текст вставится после неё.
...
Готовые образы лежат на [странице релизов](https://github.com/Fuck4ik/cyclop/releases) —
...
Календарь и диктовка — единственные вкладки, ради которых Cyclop просит
разрешения. Системные диалоги появляются не при запуске и не при открытии
вкладки, а после явного нажатия кнопки на объясняющем экране. Календарю нужен
доступ к EventKit, диктовке — Микрофон и Универсальный доступ.
...
- Куда попадёт «Копировать полный путь» в контекстном меню, решает Finder:
  пункты сторонних расширений он всегда складывает в нижний блок, после «Быстрых
  действий». Управлять этим нечем — API для позиции не существует.
...
└── FinderPathText.swift       выделение Finder -> строки путей, покрыто тестами

Sources/CyclopFinderMenu       расширение Finder, живёт в Contents/PlugIns
├── main.swift                 точка входа: NSExtensionMain
├── FinderMenu.swift           пункт меню и копирование в буфер
└── Strings.swift              локализация из собственного бандла
```
